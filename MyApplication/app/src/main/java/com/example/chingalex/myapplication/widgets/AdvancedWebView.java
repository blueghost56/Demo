package com.example.chingalex.myapplication.widgets;
...
/**
 * Created by chingalex on 2017/1/11.
 */
...
public class AdvancedWebView extends WebView {
            ...
            if (resultCode == Activity.RESULT_OK) {
                if (intent != null) {
                    ...
                    if(cameraFile.exists()){
                        ...
                        mActivity.get().sendBroadcast(new Intent(
                                Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, result));
                        mFileUploadCallbackFirst.onReceiveValue(result);
                        mFileUploadCallbackFirst=null;
                        Toast.makeText(getContext(),cameraFile.getAbsolutePath(),Toast.LENGTH_SHORT).show();
                    }
                }
            }
            else {
    ...
    protected void openFileInput(final ValueCallback<Uri> fileUploadCallbackFirst, final ValueCallback<Uri[]> fileUploadCallbackSecond, final boolean allowMultiple) {
        ...
        if (mFragment != null && mFragment.get() != null && Build.VERSION.SDK_INT >= 11) {
            mFragment.get().startActivityForResult(createDefaultOpenableIntent(i), mRequestCodeFilePicker);
        }
        else if (mActivity != null && mActivity.get() != null) {
            mActivity.get().startActivityForResult(createDefaultOpenableIntent(i), mRequestCodeFilePicker);
        }
    }

    private Intent createDefaultOpenableIntent(Intent i){
    ...
    private Intent createChooserIntent(Intent... intents){
        ...
        chooser.putExtra(Intent.EXTRA_TITLE,getFileUploadPromptLabel());
        return chooser;
    }
    ...
    private Intent createCameraIntent(){
        ...
        Log.d("ExternalDataDir:",externalDataDir.toString());
        ...
        Log.d("CamerFilePath:",mCameraFilePath);
